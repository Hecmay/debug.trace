...
//  data_t A[I][K], B[K][J], C[I][J], C_golden[I][J]; 
...

  {
    ...
    std::vector<float, aligned_allocator<float>> dev_A_unserialized((1040) * (1024));
    std::vector<float, aligned_allocator<float>> dev_A(4259840);
    std::vector<float, aligned_allocator<float>> dev_B_unserialized((1024) * (1024));
    std::vector<float, aligned_allocator<float>> dev_B(4194304);
    std::vector<float, aligned_allocator<float>> dev_C_unserialized((1040) * (1024));
    std::vector<float, aligned_allocator<float>> dev_C(1064960);

    // Initialize host buffers
    std::copy(reinterpret_cast<float *>(A), reinterpret_cast<float *>(A) + (1040) * (1024), dev_A_unserialized.begin());
    std::copy(reinterpret_cast<float *>(B), reinterpret_cast<float *>(B) + (1024) * (1024), dev_B_unserialized.begin());
    std::copy(reinterpret_cast<float *>(C), reinterpret_cast<float *>(C) + (1040) * (1024), dev_C_unserialized.begin());
